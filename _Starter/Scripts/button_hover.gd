...
@export var focus_size := 1.05
@export var focus_angle := 1
...
func hover_in():
	...
	get_tree().create_tween().tween_property(self, "rotation_degrees", randf_range(-focus_angle, focus_angle) * 3, 0.15).set_trans(Tween.TRANS_BOUNCE)
	get_tree().create_tween().tween_property(self, "scale", Vector2.ONE * focus_size, 0.15).set_trans(Tween.TRANS_BOUNCE)
	hover_sound()
